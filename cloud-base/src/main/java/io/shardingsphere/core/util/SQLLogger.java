...
public final class SQLLogger {
     ...
     */
    public static void logSQL(String logicSQL, final SQLStatement sqlStatement, final Collection<RouteUnit> routeUnits) {
        logicSQL = logicSQL.replaceAll("\n", " ").replaceAll("\\s+", " ");

        log("Rule Type: sharding");
        log("Logic SQL: {}", logicSQL);
        log("SQLStatement: {}", sqlStatement);
        for (RouteUnit each : routeUnits) {
            final String actualSql = each.getSqlUnit().getSql().replaceAll("\n", " ").replaceAll("\\s+", " ");
            if (each.getSqlUnit().getParameterSets().get(0).isEmpty()) {
                log("Actual SQL: {} ::: {}", each.getDataSourceName(), actualSql);
            } else {
                log("Actual SQL: {} ::: {} ::: {}", each.getDataSourceName(), actualSql, each.getSqlUnit().getParameterSets());
            }
        }
    }
